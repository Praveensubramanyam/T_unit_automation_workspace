...
class AdvancedPricingFeatureEngineer:
    ...
    def advanced_feature_engineering(self, df, is_training=True):
        ...
        try:
            df["MRP_Category"] = pd.cut(
                ...
                bins=[
                    -np.inf,
                    self.mrp_quantiles[0],
                    self.mrp_quantiles[1],
                    np.inf,
                ],
                labels=[0, 1, 2],
                duplicates="drop",
            ...
            df["MRP_Category"] = pd.qcut(
                df["MRP_x"].rank(method="first"), q=3, labels=[0, 1, 2]
            ).astype(int)
        ...
        if is_training:
            self.brand_mrp_median = (
                df.groupby("Brand")["MRP_x"].median().to_dict()
            )
            self.overall_brand_median = pd.Series(
                self.brand_mrp_median.values()
            ).median()

        df["Brand_MRP_Median"] = (
            df["Brand"]
            .map(self.brand_mrp_median)
            .fillna(self.overall_brand_median)
        )
        df["Is_Premium_Brand"] = (
            df["Brand_MRP_Median"] > self.overall_brand_median
        ).astype(int)
        df["MRP_vs_Brand_Median"] = df["MRP_x"] / df["Brand_MRP_Median"]

        # 4. Seasonal/Temporal features
        df["Is_Peak_Season"] = (
            df["Month"].isin([11, 12, 1]).astype(int)
        )  # Holiday season
        ...
        df["Market_Competition_Index"] = (
            1.0  # Placeholder - should be external data
        )
        df["Economic_Index"] = (
            1.0  # Placeholder - should be external economic indicators
        )
        ...
        if "Date" in df.columns and is_training:
            ...
            product_launch = (
                df.groupby(["Brand", "FC_ID"])["Date"].min().to_dict()
            )
        ...
        if hasattr(self, "product_launch_dates"):
            df["Product_Age_Days"] = (
                df["Date"]
                - df.apply(
                    lambda x: self.product_launch_dates.get(
                        (x["Brand"], x["FC_ID"]), x["Date"]
                    ),
                    axis=1,
                )
            ).dt.days
            df["Is_New_Product"] = (df["Product_Age_Days"] <= 30).astype(int)
            df["Is_Mature_Product"] = (df["Product_Age_Days"] >= 365).astype(
                int
            )
        else:
            ...
            if col in df_encoded.columns:
                if col not in self.label_encoders:
                    ...
                    df_encoded[f"{col}_encoded"] = self.label_encoders[
                        col
                    ].fit_transform(df_encoded[col].astype(str))
                else:
    ...
    def prepare_features_target(self, df):
        ...
        if len(available_features) < 5:
            print(
                "WARNING: Very few features available. Adding basic features."
            )
            basic_features = [
                "Log_MRP",
                "MRP_Category",
    ...
    def fit_transform(self, df):
        ...
        print("\n" + "=" * 60)
        print(
            "LEAKAGE CHECK PASSED - All features are available BEFORE pricing decision"
        )
        print("=" * 60)
    ...
    def transform(self, df):
        ...
        df_engineered = self.advanced_feature_engineering(
