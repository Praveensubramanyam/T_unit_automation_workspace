...
class PricingDataValidator:
    ...
    def detect_outliers(self, df: pd.DataFrame, method: str = "iqr", threshold: float = 1.5) -> pd.Series:
        ...
        if method == "iqr":
            ...
            raise ValueError("Unsupported outlier detection method. Use 'iqr' or 'zscore'.")
...
class BlobStorageManager:
    ...
    def download_blob_as_df(self, container: str, filename: str) -> pd.DataFrame:
        """Download blob as pandas DataFrame."""
        try:
            blob_client = self.blob_service_client.get_blob_client(container=container, blob=filename)
            stream = blob_client.download_blob().readall()
            return pd.read_csv(BytesIO(stream))
        except Exception as e:
            raise
    
    def upload_df_to_blob(self, df: pd.DataFrame, container: str, blob_name: str):
...
def get_price_columns(df: pd.DataFrame) -> List[str]:
    ...
    price_keywords = ['price', 'cost', 'amount', 'selling', 'revenue', 'value', 'fee']
    price_cols = []
...
class DataPreprocessor:
    ...
    def preprocess_individual_files(self):
        ...
        for filename in self.file_list:
            try:
                df = self.blob_manager.download_blob_as_df(self.input_container, filename)
                ...
                self.blob_manager.upload_df_to_blob(df, self.output_container, cleaned_name)
            except Exception as e:
                continue
    ...
    def merge_datasets(self) -> pd.DataFrame:
        ...
        try:
            sales_df = self.blob_manager.download_blob_as_df(self.output_container, "Sales_Data_cleaned.csv")
            competitor_df = self.blob_manager.download_blob_as_df(self.output_container, "Competitor_Pricing_Data_cleaned.csv")
            customer_df = self.blob_manager.download_blob_as_df(self.output_container, "Daily_Customer_Behavior_cleaned.csv")
            inventory_df = self.blob_manager.download_blob_as_df(self.output_container, "Inventory_Data_cleaned.csv")
            ...
            self.blob_manager.upload_df_to_blob(merged_df, self.merged_container, "unified_dataset.csv")
            return merged_df
        except Exception as e:
            raise
