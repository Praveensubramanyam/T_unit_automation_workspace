...
class BusinessEvaluationFramework:
    ...
    def calculate_revenue_impact(self, y_true, y_pred, units_sold):
        ...
        revenue_impact = (predicted_revenue - actual_revenue) / actual_revenue * 100
    ...
    def calculate_inventory_efficiency_impact(self, predictions, actual_demand, stock_levels):
        """Calculate inventory efficiency improvements from better pricing"""
        # Stock optimization score
        optimal_stock_ratio = np.mean(actual_demand / np.maximum(stock_levels, 1))
        
        # Stockout prevention score
        stockout_risk_reduction = np.mean(
            np.maximum(0, stock_levels - actual_demand) / np.maximum(stock_levels, 1)
        )
    ...
    def calculate_profit_optimization_score(self, prices, costs, units_sold, margins):
        ...
        return {
            ...
            'profit_per_unit': actual_profit / np.sum(units_sold) if np.sum(units_sold) > 0 else 0
        }
    
    def generate_business_report(self, y_true, y_pred, units_sold, additional_data=None):
        """Generate comprehensive business impact report"""
        # Calculate all metrics
        revenue_metrics = self.calculate_revenue_impact(y_true, y_pred, units_sold)
        accuracy_metrics = self.calculate_pricing_accuracy_metrics(y_true, y_pred)
