...
class PricingTimeSeriesAnalyzer:
    ...
    def detect_seasonality(self, df, value_col='SellingPrice'):
        ...
        dominant_periods = [1/abs(freqs[i]) for i in dominant_freq_idx if freqs[i] != 0]
    ...
    def create_lag_features(self, df, target_col='SellingPrice', lags=[1, 7, 14, 30]):
        """Create lag features for time series modeling"""
        df = df.copy()
        df = df.sort_values(['Brand', 'FC_ID', 'Date'])
        
        for lag in lags:
            df[f'{target_col}_lag_{lag}'] = df.groupby(['Brand', 'FC_ID'])[target_col].shift(lag)
    ...
    def forecast_demand(self, df, forecast_horizon=30):
        ...
        for brand in df['Brand'].unique():
            for fc_id in df['FC_ID'].unique():
                subset = df[(df['Brand'] == brand) & (df['FC_ID'] == fc_id)].copy()
                ...
                if len(recent_demand) > 3:
                    ...
                    # Generate forecast
                    forecast = [last_value + trend * i for i in range(1, forecast_horizon + 1)]
